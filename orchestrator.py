...
class LeadFactory:
    ...
    def discover_lane(self, run_id: str, lane: str) -> dict:
        ...
        # Bootstrap sources are deterministic fuel; LLM discovery is supplemental.
    ...
    def _autonomy_guard(self, lane: str) -> dict | None:
        ...
        if str(self.s.autonomy_mode or "").upper() != "UNTIL_TARGET":
            return None
        # A live Qualified-Lead Production SLO owns stopping decisions. The
        # legacy fixed-count/zero-yield guard must not terminate its frontier.
        goal_status = self._config().get("LEAD_FACTORY_GOAL_STATUS", "").upper()
        if goal_status in {"RUNNING", "AT_RISK"}:
            return None
