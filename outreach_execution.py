...
CRITICAL_FIELDS = {"recipient", "subject", "body", "company_name", "lane"}
...
def is_sacrificial_lane(row: dict, cfg: dict[str, str]) -> bool:
    allowed = {item.strip().upper() for item in str(cfg.get("OUTREACH_SACRIFICE_LANES", "EC,RETAIL,SACRIFICE") or "").split(",") if item.strip()}
    lane = lane_from(row)
...
def semantic_email_preflight(row: dict, cfg: dict[str, str]) -> dict:
    ...
    # URLs are valid in email bodies; URL restrictions belong to form-field validation.
    ...
    return {"ok": not missing and not errors, "missing": missing, "critical_errors": errors, "message_hash": message_hash(row)}
...
class SacrificialEmailExecutor:
    ...
    def execute(self, draft: dict, cfg: dict[str, str]) -> dict:
        ...
        existing = self.sheets._rows_as_dicts("LeadFactory_ExecutionLog", "ZZ")
        ...
        self.sheets.append_dict("LeadFactory_ExecutionLog", {
            "idempotency_key": key, "draft_id": draft.get("draft_id", ""),
            "company_name": draft.get("company_name", ""), "lane": lane_from(draft),
            "channel": "EMAIL", "status": "SENT", "semantic_success": "PENDING_DELIVERY",
            "message_id": result.get("id", ""), "executed_at": now,
        })
