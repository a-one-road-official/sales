...
Fetch `ssot_terminal.py`, `outreach_master.py`, `customer_care.py` and `prompts/japan_outreach_master.txt` from the same current main commit. Preserve those relative paths in the current ChatGPT Python/container environment; these modules use the standard library. Record the commit used. Do not combine a cached older validator with a newer terminal or fabricate a missing helper. Execute with private connected-tool data. Do not send input JSON to a hosted executor or publish it to GitHub.
...
5. Generate and verify the actual original copy in this ChatGPT run. Packet: company_id, website, generator=chatgpt_master_agent, draft={subject,body,master_prompt_hash}, generated_at, email_sha256=SHA256(subject+'\n'+body), evidence=[{url,excerpt,source_text,relevance}, ...], review with all actual checks from REVIEW_KEYS, and customer_review/recipient_evidence as defined below. Use original generated_at; an evidence-only recheck may update rechecked_at without regenerating unchanged copy.
